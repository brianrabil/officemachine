...
pub const canvas_label = "music-canvas";
...
const shell_views = [_]native_sdk.ShellView{
    .{ .label = canvas_label, .kind = .gpu_surface, .fill = true, .role = "Music library canvas", .accessibility_label = "Music library", .gpu_backend = .metal, .gpu_pixel_format = .bgra8_unorm, .gpu_present_mode = .timer, .gpu_alpha_mode = .@"opaque", .gpu_color_space = .srgb, .gpu_vsync = true },
};
...
pub fn main(init: std.process.Init) !void {
    ...
    // (~/Library/Caches/music-player on macOS, XDG cache on Linux) hosts
    ...
    try runner.runWithOptions(app_state.app(), .{
        ...
        .window_title = "Music",
        .bundle_id = "dev.officemachine.music",

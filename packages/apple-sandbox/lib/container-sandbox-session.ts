...
export interface ContainerActivityTracker {
  touch(): void;
}

/** `Experimental_SandboxSession` backed by a running Apple `container` instance. Every operation shells out to `container exec`. */
export class ContainerSandboxSession implements SandboxSession {
  constructor(
    ...
    protected readonly activityTracker?: ContainerActivityTracker,
  ) {}
  ...
  async spawn({
    ...
    // `container exec`'s client can outlive the process it started (same detached-process
    // gap `docker exec` has), so record the real PID for kill() to fall back to.
...
function createSandboxProcess(input: {
  ...
  // @types/node's stream/web ReadableStream<any> doesn't structurally match the DOM lib's
  // ReadableStream<Uint8Array> even though they're the same runtime class.
...
const BUFFER_ENCODINGS: Record<BufferEncoding, true> = {
  ascii: true,
  utf8: true,
  "utf-8": true,
  utf16le: true,
...
function assertBufferEncoding(encoding: string): asserts encoding is BufferEncoding {
  if (!(encoding in BUFFER_ENCODINGS)) {

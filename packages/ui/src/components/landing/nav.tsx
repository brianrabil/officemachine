...
export function Nav({ brand, brandHref = "/", links, githubUrl }: NavProps) {
      ...
      <div className="mx-auto max-w-[1320px]">
        <div
          className={`flex h-16 items-center justify-between border-x border-b px-6 transition-colors duration-200 ${
            scrolled
              ? "border-x-border border-b-border bg-background/95 backdrop-blur-sm"
              : "border-x-transparent border-b-transparent bg-transparent"
          }`}
        >
          <Link href={brandHref} className="flex items-center gap-2.5 text-foreground">
            <svg
              viewBox="0 0 24 24"
              className="size-5"
              fill="none"

...
/**
 * Options every helper in this module accepts for locating and invoking the
 * `container` binary (https://github.com/apple/container). Threaded through
 * from {@link ContainerSandboxSettings.binaryPath} so a caller can point at a
 * non-PATH install without every call site re-declaring the option.
 */
...
/**
 * Thrown when the `container` binary itself cannot be found (`ENOENT` from
 * `spawn`) — distinct from the binary running and the *command* failing,
 * which is {@link ContainerCommandError}.
 */
...
/**
 * Runs `container <args>` to completion and collects its output as raw
 * bytes (never text-decoded), so callers doing binary file I/O don't lose
 * data to an intermediate encoding step. Never rejects for a non-zero exit —
 * only for a spawn-level failure (e.g. the binary not being found, which
 * rejects with {@link ContainerCliNotFoundError}).
 */
...
/**
 * Spawns `container <args>` without waiting for it to finish, for
 * long-running/streaming use (backing {@link SandboxSession.spawn}). Stdin is
 * closed immediately — spawned commands are wrapped so they don't need it
 * (see `container-sandbox-session.ts`).
 */
...
/**
 * Derives a `container`-safe resource name from an arbitrary session
 * identifier (e.g. a UUID or an opaque harness session id), so names built
 * from caller-supplied ids never trip the CLI's naming rules.
 */
...
/** Builds the argv for `container exec ...` used by every per-call operation on a live session. */

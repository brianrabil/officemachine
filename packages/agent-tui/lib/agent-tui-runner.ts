...
/**
 * A lighter-weight agent shape for callers that already have a
 * `UIMessageChunk` stream (e.g. proxying `apps/api`'s chat endpoints) and
 * would otherwise have to round-trip it through `convertToModelMessages` +
 * `agent.stream()`'s `TextStreamPart`-shaped `fullStream` just to convert it
 * straight back to `UIMessageChunk`s again inside this runner. `renderStream`
 * (see `tui/terminal-renderer.ts`) already accepts a raw `UIMessageChunk`
 * stream directly via `AgentTUIStreamResult.uiMessageStream` — this type lets
 * a caller hand that over untouched.
 */
export type UIMessageStreamAgent = {
  tools?: ToolSet;
...
export type AgentTUIRenderer = {
  ...
  /**
   * Renders already-complete messages (e.g. history fetched on resume)
   * directly into the renderer's own scrollback, without going through the
   * streaming path — there is nothing to stream, they already happened.
   */
  renderHistory?(messages: UIMessage[]): void;
};

export type AgentTUIRunnerOptions<
  TAgent extends AgentTUIAgent | UIMessageStreamAgent = AgentTUIAgent,
> = Omit<RunAgentTUIOptions, 'agent'> & {
  agent: TAgent;
  /**
   * Prior messages to seed the conversation with (e.g. persisted history
   * when resuming an existing chat elsewhere). Included in the context sent
   * on the next turn and, if the renderer supports it, painted into the
   * scrollback immediately via `renderHistory`.
   */
...
> {
  ...
  async run() {
    ...
    if (messages.length > 0) {
      this.renderer.renderHistory?.(messages);
    }
    ...
    while (true) {
      ...
      let result: AgentTUIStreamResult;
      try {
        result = await this.streamMessages([...messages], generateMessageId);
      } catch (error) {
        if (isInterruptedError(error)) {
          return;
        }
        ...
        result = {
          uiMessageStream: (async function* () {
            yield { type: 'error', errorText } satisfies UIMessageChunk;
          })(),
        };
      }
  ...
  private async streamMessages(
    ...
    if (isUIMessageStreamAgent(this.agent)) {
      // Already speaks UIMessageChunk (e.g. proxying an HTTP endpoint that
      // streams it directly) — nothing to convert in either direction.
      return this.agent.streamMessages({
        messages,
...
function createDefaultRenderer(options: Omit<AgentTUIRunnerOptions, 'agent'>) {
...
function createRenderer(
  options: Omit<AgentTUIRunnerOptions, 'agent'>,

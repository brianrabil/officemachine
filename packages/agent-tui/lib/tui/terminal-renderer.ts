...
export class TerminalRenderer {
  ...
  /**
   * Paints already-complete messages directly into the scrollback — used for
   * history seeded via `AgentTUIRunnerOptions.initialMessages` (e.g. when
   * resuming an existing chat fetched from elsewhere). There is nothing to
   * stream for these; they already happened.
   */
  renderHistory(messages: UIMessage[]) {
    const displayModes = {
      tools: this.#tools,

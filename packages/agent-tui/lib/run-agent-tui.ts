import { AgentTUIRunner, type UIMessageStreamAgent } from './agent-tui-runner';
import type { Agent, Experimental_SandboxSession, UIMessage } from 'ai';

export type { UIMessageStreamAgent } from './agent-tui-runner';
...
export type RunAgentTUIOptions = {
  /**
   * The agent to run. Either a full AI SDK `Agent` or a lighter-weight
   * {@link UIMessageStreamAgent} for callers that already have a
   * `UIMessageChunk` stream (e.g. proxying an HTTP chat endpoint).
   */
  agent: AgentTUIAgent | UIMessageStreamAgent;

  /**
   * Prior messages to seed the conversation with — e.g. history fetched when
   * resuming an existing chat. Painted into the scrollback immediately (if
   * the renderer supports it) and included in context for the next turn.
   */
